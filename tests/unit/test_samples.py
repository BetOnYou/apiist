...
class TestSamples(TestCase):
    def test_transactions(self):
        ...
        nose2.discover(
            argv=[__file__, "tests.resources.test_transactions", "-v"],
            module="None",
            exit=False,
        ...
        self.assertEqual(
            "test_3_nested_transactions.outer", middle.test_suite + "." + middle.test_case
        )
        ...
        self.assertEqual(request.test_suite, "test_5_apiist_assertions")
        ...
        self.assertEqual(request.test_suite, "test_6_apiist_assertions_failed")
        self.assertEqual(request.test_case, "http://blazedemo.com/")
        self.assertEqual(len(request.assertions), 1)
        assertion = request.assertions[0]
        ...
        self.assertEqual(
            assertion.error_message, "Request to https://blazedemo.com/ didn't fail (200)"
        )
    ...
    def test_label_single_transaction(self):
        ...
        store.writer = CachingWriter()
        nose2.discover(
            argv=[__file__, "tests.resources.test_single_transaction", "-v"],
            module="None",
            exit=False,
            plugins=["tests.unit.test_samples"],
        )
    ...
    def test_two_transactions(self):
        ...
        store.writer = CachingWriter()
        nose2.discover(
            argv=[__file__, "tests.resources.test_two_transactions", "-v"],
            module="None",
            exit=False,
            plugins=["tests.unit.test_samples"],
        )
